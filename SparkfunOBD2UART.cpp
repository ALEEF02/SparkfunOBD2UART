/*************************************************************************
* Arduino Library for Sparkfun OBD-II UART Adapter
* Licensed under the MIT License
* Anthony Ford <dev@aleef.dev>
*************************************************************************/
...
byte COBD::readDTC(uint16_t codes[], byte maxCodes)
{
	...
	*/ 
	byte codesRead = 0;
 	for (byte n = 0; n < 6; n++) {
		char buffer[128];
		sprintf_P(buffer, n == 0 ? PSTR("03") : PSTR("03%02X"), n);
		write(buffer);
...
{
  	char buf[32];
	sendCommand("AT LP", buf, sizeof(buf));
}
...
void COBD::leaveLowPowerMode()
{
	...
	char buf[32];
	sendCommand("AT I", buf, sizeof(buf), 1000);
}
...
{
    char buf[32];
	if (sendCommand("AT RV", buf, sizeof(buf)) > 0) {
...
{
	for (byte n = 0; n < 5; n++) {
		if (sendCommand("0902", buffer, bufsize)) {
...
byte COBD::begin()
{
	long baudrates[] = {115200, 38400, 9600};
	byte version = 0;
...
byte COBD::getVersion()
{
	...
	for (byte n = 0; n < 3; n++) {
		char buffer[32];
		if (sendCommand("AT I", buffer, sizeof(buffer), 200)) {
...
int COBD::init(OBD_PROTOCOLS protocol, byte unitFormat)
{
	...
	for (byte n = 0; n < 2; n++) {
		...
		stage = 2;
		delay(10);
		if (!sendCommand("010D", buffer, sizeof(buffer), OBD_TIMEOUT_LONG) || checkErrorMessage(buffer)) {
			continue;
		}
		stage = 3;
		// load pid map
		...
		for (byte i = 0; i < 4; i++) {
			byte pid = i * 0x20;
			sprintf(buffer, "%02X%02X", dataMode, pid);
			delay(10);
			write(buffer);
			delay(10);
...
void COBD::reset()
{
	...
	delay(3000);
	sendCommand("AT Z", buf, sizeof(buf));
}

void COBD::uninit()
{
	char buf[32];
	sendCommand("AT PC", buf, sizeof(buf));
}
